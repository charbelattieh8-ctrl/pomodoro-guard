...
export default function RequestsPanel({ incoming, onAccept, onDecline, userMap = {} }) {
  return (
    <GlassCard className="space-y-3 p-4">
      ...
      {incoming.map((r) => (
        <div key={r.id} className="flex items-center justify-between rounded-xl border border-white/15 bg-white/5 px-3 py-2">
          <p className="text-sm">
            Request from @{userMap[r.fromUid]?.username || userMap[r.fromUid]?.displayName || r.fromUid.slice(0, 6)}
          </p>
