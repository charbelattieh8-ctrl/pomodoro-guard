...
/* ── Wind streams sweeping across scene ─────────────────────
   Each stream is a wide blurred horizontal band animated
   from left to right. Staggered delays create natural gusts. */
const WIND_STREAMS = [
  { left: "-20%", top: "14%", width: "28rem", delay: "0s",   duration: "18s", opacity: 0.26, blur: "7px"  },
  { left: "-18%", top: "31%", width: "22rem", delay: "3.4s", duration: "22s", opacity: 0.18, blur: "10px" },
  { left: "-24%", top: "47%", width: "26rem", delay: "7.2s", duration: "20s", opacity: 0.16, blur: "11px" },
  { left: "-16%", top: "63%", width: "30rem", delay: "1.4s", duration: "19s", opacity: 0.2,  blur: "8px"  },
];

/* ── Saltating sand grains ───────────────────────────────────
   Individual grains arc through the air (saltation).
   CSS custom properties drive the parabolic path. */
const SAND_GRAINS = [
  ...
  return (
    <div
      ...
      style={buildSceneVars(theme, {
        ...
        "--scene-dune-texture": `url(${THEME_SCENE_ASSETS.sand.duneTexture})`,
        "--scene-normal":       `url(${THEME_SCENE_ASSETS.sand.normalMap})`,
      })}
    >
      {/* ── Background: sky, sun, atmosphere ── */}
      <div className="sand-sky absolute inset-0" />
      <div className="sand-sun absolute right-[-8%] top-[-6%] h-[42vh] w-[42vh] rounded-full" />

      {/* Heat shimmer — wavy distortion above sand */}
      {!reduceMotion ? (
        <div className="sand-heat-shimmer absolute inset-x-0 top-[28%] h-[40%]" />
      ) : null}

      <div className="sand-distance absolute inset-x-0 top-[12%] h-[26%]" />
      <div className="sand-atmosphere absolute inset-x-0 top-[18%] h-[28%]" />

      {/* Distant horizon ridge silhouettes */}
      <div className="absolute inset-0">
        {DISTANT_RIDGES.map((ridge) => (
          <span
            key={ridge.className}
            className={`sand-ridge absolute ${ridge.className}`}
            style={{ left: ridge.left, top: ridge.top, width: ridge.width, height: ridge.height, clipPath: ridge.clipPath }}
          />
        ))}
      </div>
      ...
      >
        {/* Real sand texture body */}
        <div className="sand-body absolute inset-0" />
        {/* Normal-map micro-ripple detail overlay */}
        <div className="sand-body-normal absolute inset-0" />
        {/* Color noise / light variation */}
        <div className="sand-body-noise absolute inset-0" />

        {/* ── Dune mounds at surface top ── */}
        ...
        {/* Saltating grains launched from crest */}
        {!reduceMotion ? (
          <div className="sand-crest-saltation absolute inset-x-0 top-[-4rem] h-28">
            {SAND_GRAINS.slice(0, 8).map((grain, i) => (
              <span
                key={`sand-crest-grain-${i}`}
